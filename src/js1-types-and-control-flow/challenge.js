...
export const getIsValidOnNameTag = () => {
  // Write your code here
  // console.log(nameTagOption[0].toUpperCase());
  // console.log(nameTagOption[0]==nameTagOption[0].toUpperCase);
  if (nameTagOption.length<=8 && nameTagOption.slice(0,1)==nameTagOption[0].toUpperCase()) {
    return true ;
  } else {
    return false;
  }
...
export const getHasUppercaseLetters = () => {
  // Write your code here
  //let regex = /^([A-Z][a-z]+)+$/;
  if (stringWithUppercaseLetters.match(/([A-Z a-z ])\w+/))
  return true;
  else
  return false ;
};
