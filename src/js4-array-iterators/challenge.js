...
export const removeFalseValues = (booleanArr) => {
  return booleanArr.filter(element => element == true);
};
...
export const createPercentageList = (numbersArr) => {
  return numbersArr.map(element=> `${element*100}%`);
}
/**
...
export const createListOfPoessessions = (possessionsArr, name) => {
  return possessionsArr.map(element=>name+" "+element);
};
...
export const convertStringToNumbersArray = (numberString) => {
  return numberString.split("+").map(element=>parseInt(element));
};
...
export const createOddEvenArray = (numberString) => {
  return numberString.split("+").map(element=>(parseInt(element)%2?`even`:`odd`))
};
